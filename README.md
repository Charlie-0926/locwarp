...
路線巡迴 / 多點導航中可勾「**點對點跳躍**」,改成逐點瞬移、不再走 OSRM 路徑。所有等待都發生在到站後；「**到站後先等待**」預設為 6 秒，設定會記在 localStorage。

- 開啟「**等待後直線移動**」時，每次到站先原地等待 `n` 秒，再隨機選一個固定方向，以 15 km/h 直線移動使用者設定的 `m` 秒。移動時間預設 4 秒，位移約為 `4.1667×m` 公尺，每站總時間為 `n+m` 秒。
- 關閉直線移動時，只在到站位置原地等待 `n` 秒。
- 暫停會凍結移動與等待倒數；hot apply 從下一個停靠點起生效。單趟最後一點及有限圈數的完成起點會停在精確路徑點，不再執行到站後移動。
